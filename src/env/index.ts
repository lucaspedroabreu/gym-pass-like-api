...
const passcodesVariables = z.object({
  PASSWORD_HASHING_PEPPER: z.string().min(1, 'Must have a PASSWORD_HASHING_PEPPER variable set'),
  JWT_SECRET: z.string().min(1, 'Must have a JWT_SECRET variable set'),
})

// Merging server and database configuration schemas into a global environment schema
const globalEnvSchema = nodeServerEnv.merge(databaseConfigEnv).merge(passcodesVariables)
